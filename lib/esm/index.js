 ...
 * @returns
 */
export async function getAptosTx(client, user, method, args, type_args = []) {
    const payload = {
        type: "entry_function_payload",
        function: method,
        type_arguments: type_args,
...
export class AggregatorAccount {
    ...
    async openRound(account) {
        return await sendAptosTx(this.client, account, `${this.switchboardAddress}::aggregator_open_round_action::run`, [HexString.ensure(this.address).hex()], [this.coinType]);
    }
    async openRoundTx(accountAddress) {
        var _a;
        return await getAptosTx(this.client, accountAddress, `${this.switchboardAddress}::aggregator_open_round_action::run`, [HexString.ensure(this.address).hex()], [(_a = this.coinType) !== null && _a !== void 0 ? _a : "0x1::aptos_coin::AptosCoin"]);
    }
...
export class LeaseAccount {
    ...
    async extendTx(account, params) {
        return await getAptosTx(this.client, account, `${this.switchboardAddress}::lease_extend_action::run`, [HexString.ensure(this.address).hex(), params.loadAmount], [this.coinType]);
    }
    /**
     * Pop an aggregator off the Crank
     */
    ...
    async withdrawTx(account, params) {
        return await getAptosTx(this.client, account, `${this.switchboardAddress}::lease_withdraw_action::run`, [[HexString.ensure(this.address).hex(), params.amount]], [this.coinType]);
    }
    async loadData() {
...
export class OracleWallet {
    ...
    static async init(client, account, params, switchboardAddress) {
        var _a, _b;
        const tx = await sendAptosTx(client, account, `${switchboardAddress}::oracle_wallet_init_action::run`, [HexString.ensure(params.oracleAddress).hex()], [(_a = params.coinType) !== null && _a !== void 0 ? _a : "0x1::aptos_coin::AptosCoin"]);
        return [
