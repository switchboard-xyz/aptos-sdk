...
class Aggregator {
    ...
    async loadJobs() {
        ...
        const jobs = data.job_keys.map((key) => new Job(this.client, aptos_1.HexString.ensure(key).hex(), aptos_1.HexString.ensure(this.devnetAddress).hex()));
        const promises = [];
        for (let job of jobs) {
...
async function createFeed(client, account, params, devnetAddress) {
    ...
    return [
        new Aggregator(client, resource_address, devnetAddress, (_j = params.coinType) !== null && _j !== void 0 ? _j : "0x1::aptos_coin::AptosCoin"),
        tx,
    ];
}
