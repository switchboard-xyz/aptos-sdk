...
class State {
    ...
    static async init(client, account, pid) {
        const tx = await sendAptosTx(client, account, {
            module: {
                address: aptos_1.HexString.ensure(pid).hex(),
    ...
    async loadData() {
        return (await this.client.getAccountResource(this.address, {
            address: aptos_1.HexString.ensure(this.devnetAddress).hex(),
            module: "Switchboard",
            name: "State",
            generic_type_params: [],
        })).data;
...
class Aggregator {
    ...
    async loadData() {
        return (await this.client.getAccountResource(aptos_1.HexString.ensure(this.address).hex(), {
            address: aptos_1.HexString.ensure(this.devnetAddress).hex(),
            module: "Aggregator",
            name: "Aggregator",
            generic_type_params: [],
    ...
    static async init(client, account, params, devnetAddress, stateAddress) {
        ...
        const tx = await sendAptosTx(client, account, {
            module: {
                address: aptos_1.HexString.ensure(devnetAddress).hex(),
        ...
        return [
            new Aggregator(client, account.address(), devnetAddress, stateAddress),
            tx,
        ];
    }
    async addJob(account, params) {
        return await sendAptosTx(this.client, account, {
            module: {
                address: aptos_1.HexString.ensure(this.devnetAddress).hex(),
    ...
    async watch(callback) {
        const event = new AptosEvent(this.client, aptos_1.HexString.ensure(`${this.devnetAddress}::Switchboard::State`), {
            address: aptos_1.HexString.ensure(this.devnetAddress).hex(),
            module: "Switchboard",
            name: "State",
            generic_type_params: [],
        }, "aggregator_update_events", 1000);
