 ...
 * @returns
 */
function getAptosTx(method, args, type_args = []) {
    const payload = {
        type: "entry_function_payload",
        function: method,
        type_arguments: type_args,
...
class AggregatorAccount {
    ...
    static getLeaseAddress(aggregatorAddress, queueAddress) {
        return generateResourceAccountAddress(aptos_1.HexString.ensure(aggregatorAddress), bcsAddressToBytes(aptos_1.HexString.ensure(queueAddress)));
    }
    async leaseAddress() {
    ...
    static async init(client, account, params, switchboardAddress) {
        ...
        const { mantissa: vtMantissa, scale: vtScale } = AptosDecimal.fromBig((_a = params.varianceThreshold) !== null && _a !== void 0 ? _a : new big_js_1.default(0));
        const seed = new aptos_1.AptosAccount().address();
        const resource_address = generateResourceAccountAddress(aptos_1.HexString.ensure(account.address()), bcsAddressToBytes(aptos_1.HexString.ensure(seed)));
        const tx = await sendAptosTx(client, account, `${switchboardAddress}::aggregator_init_action::run`, [
            ...
            params.gasPriceFeed
                ? aptos_1.HexString.ensure(params.gasPriceFeed).hex()
                : "0x0",
            aptos_1.HexString.ensure(params.authority).hex(),
            aptos_1.HexString.ensure(seed).hex(),
        ], [(_l = params.coinType) !== null && _l !== void 0 ? _l : "0x1::aptos_coin::AptosCoin"]);
        return [
            new AggregatorAccount(client, resource_address, switchboardAddress, (_m = params.coinType) !== null && _m !== void 0 ? _m : "0x1::aptos_coin::AptosCoin"),
            tx,
        ];
    }
    async latestValue() {
        const data = await this.loadData();
        return new AptosDecimal(data.latest_confirmed_round.result.value, 9, Boolean(data.latest_confirmed_round.result.neg))
            .toBig()
            .toNumber();
    }
    ...
    async openRound(account, jitter) {
        return await sendAptosTx(this.client, account, `${this.switchboardAddress}::aggregator_open_round_action::run`, [aptos_1.HexString.ensure(this.address).hex(), jitter !== null && jitter !== void 0 ? jitter : 1], [this.coinType]);
    }
    openRoundTx() {
        var _a;
        return getAptosTx(`${this.switchboardAddress}::aggregator_open_round_action::run`, [aptos_1.HexString.ensure(this.address).hex()], [(_a = this.coinType) !== null && _a !== void 0 ? _a : "0x1::aptos_coin::AptosCoin"]);
    }
    setConfigTx(params) {
        var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l;
        const { mantissa: vtMantissa, scale: vtScale } = AptosDecimal.fromBig((_a = params.varianceThreshold) !== null && _a !== void 0 ? _a : new big_js_1.default(0));
        const tx = getAptosTx(`${this.switchboardAddress}::aggregator_set_configs_action::run`, [
            aptos_1.HexString.ensure(this.address).hex(),
            (_b = params.name) !== null && _b !== void 0 ? _b : "",
            (_c = params.metadata) !== null && _c !== void 0 ? _c : "",
            aptos_1.HexString.ensure(params.queueAddress).hex(),
...
class JobAccount {
    ...
    async loadJob() {
        ...
        const job = common_1.OracleJob.decodeDelimited(Buffer.from(Buffer.from(data.data.slice(2), "hex").toString(), "base64"));
        return job;
    }
    /**
...
class OracleQueueAccount {
    ...
    static async init(client, account, params, switchboardAddress) {
        ...
        return [
            new OracleQueueAccount(client, account.address(), switchboardAddress, (_c = params.coinType) !== null && _c !== void 0 ? _c : "0x1::aptos_coin::AptosCoin"),
            tx,
        ];
    }
...
class LeaseAccount {
    ...
    static async init(client, account, params, switchboardAddress) {
        var _a, _b;
        let leaseAddr = generateResourceAccountAddress(aptos_1.HexString.ensure(params.aggregatorAddress), bcsAddressToBytes(aptos_1.HexString.ensure(params.queueAddress)));
        ...
        return [
            new LeaseAccount(client, leaseAddr, switchboardAddress, (_b = params.coinType) !== null && _b !== void 0 ? _b : "0x1::aptos_coin::AptosCoin"),
            tx,
        ];
    }
    /**
    ...
    extendTx(account, params) {
        return getAptosTx(`${this.switchboardAddress}::lease_extend_action::run`, [aptos_1.HexString.ensure(this.address).hex(), params.loadAmount], [this.coinType]);
    }
    /**
     * Pop an aggregator off the Crank
     */
    ...
    withdrawTx(account, params) {
        return getAptosTx(`${this.switchboardAddress}::lease_withdraw_action::run`, [[aptos_1.HexString.ensure(this.address).hex(), params.amount]], [this.coinType]);
    }
    /**
    ...
    async setAuthority(account, params) {
        return await sendAptosTx(this.client, account, `${this.switchboardAddress}::lease_set_authority_action::run`, [
            aptos_1.HexString.ensure(this.address).hex(),
            aptos_1.HexString.ensure(params.authority).hex(),
        ], [this.coinType]);
    }
...
class Permission {
    ...
    static async init(client, account, params, switchboardAddress) {
        const tx = await sendRawAptosTx(client, account, `${switchboardAddress}::permission_init_action::run`, [
            ...
            aptos_1.BCS.bcsSerializeBytes(aptos_1.HexString.ensure(params.grantee).toUint8Array()),
        ]);
...
async function fetchAggregators(client, authority, switchboardAddress) {
    ...
    const tableItems = await client.getTableItem(handle, {
        ...
        key: aptos_1.HexString.ensure(authority).hex(),
    });
